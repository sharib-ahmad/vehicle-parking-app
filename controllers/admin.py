...
from models import (IST, AdminSearchForm, CsrfOnlyForms, DeleteParkingSpotForm,
                    EditParkingLotForm, ParkingLot, ParkingLotForm, ParkingSpot,
                    ReservedParkingSpot, SpotStatus, User, UserRole, Vehicle,
                    db)
...
def view_delete_spot(spot_id):
    ...
    if form.validate_on_submit():
        if spot.status == SpotStatus.OCCUPIED:
            ...
            current_app.logger.warning(f'Attempt to delete occupied spot {spot.id} by admin {current_user.id}')
            return redirect(url_for('admin.parking_lots'))
        ...
        try:
            ...
            current_app.logger.info(f'Spot {spot.id} deleted by admin {current_user.id}')
            return redirect(url_for('admin.parking_lots'))
...
def occupied_spot_details(spot_id):
    ...
    return render_template('admin/occupied_parking_spot_details.html', reservation=reservation, est_cost=est_cost)
...
def search():
    ...
    if search_location:
        ...
        if category in ['location', 'pin_code']:
            ...
            vehicle = Vehicle.query.filter(Vehicle.vehicle_number.ilike(f"%{search_location}%")).first()

    form.search_value.data = search_location
    return render_template("admin/search.html", form=form, lots=lots, search_location=search_location, user=user, vehicle=vehicle)
...
def revenue_chart():
    ...
    if not any(revenues):
        ...
        ax.text(0.5, -0.05, f"Total Revenue: ₹{total_revenue:.2f}",
                ha='center', va='center',
                fontsize=12,
...
def parking_summary_chart():
    ...
    ax.bar(x, available_counts, width=0.4,
           label='Available', color='green', align='center')
    ax.bar(x, occupied_counts, width=0.4,
           bottom=available_counts, label='Occupied', color='red', align='center')
