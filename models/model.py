...
class User(db.Model, UserMixin):
    ...
    def schedule_deletion(self):
        self.scheduled_delete_at = datetime.now(IST) + timedelta(days=15)
        self.is_active = False

    def cancel_deletion(self):
        self.scheduled_delete_at = None
        self.is_active = True

    def should_be_deleted(self):
        return (
            self.scheduled_delete_at and datetime.now(IST) >= self.scheduled_delete_at
        )
        ...
        if not User.query.filter_by(role=UserRole.ADMIN).first():
            try:
                ...
                current_app.logger.info(
                    f"Admin created successfully: {admin.full_name}"
                )
            except SQLAlchemyError:
                db.session.rollback()
                current_app.logger.error(
                    "Error occurred during automatic admin creation"
                )
            finally:
                db.session.close()
